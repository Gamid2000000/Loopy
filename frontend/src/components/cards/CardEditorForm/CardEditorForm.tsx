...
export function CardEditorForm({
  ...
  error?: string | null;
  edit?: boolean;
  onSubmit: (value: CreateCardRequest | UpdateCardRequest) => void;
  onCancel: () => void;
}) {
  ...
  const errors = useMemo(
    () => ({
      front: !values.front.trim()
        ...
        : values.front.trim().length > 500
          ? "Не более 500 символов"
          : "",
      back: !values.back.trim()
        ? "Введите обратную сторону"
        : values.back.trim().length > 2000
  ...
  const dirty =
    values.front.trim() !== initial.front.trim() ||
    values.back.trim() !== initial.back.trim() ||
    normalizeOptional(values.example) !== normalizeOptional(initial.example) ||
    normalizeOptional(values.note) !== normalizeOptional(initial.note);
  const submit = () => {
    setSent(true);
    ...
    if (!edit) {
      onSubmit({
        front: values.front.trim(),
        back: values.back.trim(),
        example: normalizeOptional(values.example),
    ...
    if (normalizeOptional(values.example) !== normalizeOptional(initial.example))
      request.example = normalizeOptional(values.example);
    if (normalizeOptional(values.note) !== normalizeOptional(initial.note))
      request.note = normalizeOptional(values.note);
    onSubmit(request);
  };
  const field = (key: keyof Values, label: string, max: number) => (
    <Textarea
      ...
      error={sent ? errors[key] : undefined}
    />
  );
  return (
    <form
    ...
    >
      {field("front", "Лицевая сторона", 500)}
      {field("back", "Обратная сторона", 2000)}
      {field("example", "Пример", 3000)}
      {field("note", "Заметка", 3000)}
      {error && (
      ...
      <div className={styles.actions}>
        ...
        <Button type="submit" loading={loading} disabled={loading || !valid || (edit && !dirty)}>
