...
public interface CardReviewStateRepository extends JpaRepository<CardReviewState, Long> {
	...
	Optional<CardReviewState> findWithLockByUserIdAndCardId(Long userId, Long cardId);
	...
	@Query("""
			...
			and s.dueAt <= :now
			""")
	long countDueForDashboard(Long userId, DeckStatus deckStatus, CardStatus cardStatus, Instant now);
	...
	@Query("""
			...
			and s.consecutiveCorrectCount = 0
			""")
	long countNewForDashboard(Long userId, DeckStatus deckStatus, CardStatus cardStatus);
	...
	@Query("""
			...
			and s.dueAt > :now
			""")
	long countScheduledForDashboard(Long userId, DeckStatus deckStatus, CardStatus cardStatus,
			Instant now);
}
