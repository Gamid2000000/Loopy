...
public class DeckService {
	...
	public DeckResponse create(UserPrincipal principal, CreateDeckRequest request) {
		...
		Deck deck = Deck.builder().owner(owner).name(requiredName(request.getName()))
				...
				.isPublic(Boolean.TRUE.equals(request.getIsPublic())).status(DeckStatus.ACTIVE)
				.build();
	...
	public List<DeckSummaryResponse> getCurrentUserDecks(UserPrincipal principal) {
		return deckRepository
				.findAllByOwnerIdAndStatusOrderByUpdatedAtDesc(currentUser(principal).getId(),
						DeckStatus.ACTIVE)
	...
	public DeckResponse update(UserPrincipal principal, Long deckId, UpdateDeckRequest request) {
		if (!request.hasChanges())
			throw new DeckUpdateEmptyException(
					HttpResponseMessage.HTTP_DECK_UPDATE_EMPTY.getMessage());
		...
		if (deck.getStatus() == DeckStatus.ARCHIVED) {
			throw new DeckStateConflictException(
					HttpResponseMessage.HTTP_DECK_ARCHIVED_CANNOT_UPDATE.getMessage());
		}
		if (request.isNamePresent())
			deck.setName(requiredName(request.getName()));
		if (request.isDescriptionPresent())
			deck.setDescription(normalizeDescription(request.getDescription()));
		if (request.isPublicPresent()) {
			if (request.getIsPublic() == null)
				throw new IllegalArgumentException("isPublic must be true or false");
	...
	public void archive(UserPrincipal principal, Long deckId) {
		...
		if (deck.getStatus() == DeckStatus.ARCHIVED) {
			throw new DeckStateConflictException(
					HttpResponseMessage.HTTP_DECK_ALREADY_ARCHIVED.getMessage());
		}
	...
	public DeckResponse restore(UserPrincipal principal, Long deckId) {
		...
		if (deck.getStatus() == DeckStatus.ACTIVE) {
			throw new DeckStateConflictException(
					HttpResponseMessage.HTTP_DECK_ALREADY_ACTIVE.getMessage());
		}
	...
	private String requiredName(String name) {
		...
		if (normalized.length() > 100)
			throw new IllegalArgumentException("Deck name must not exceed 100 characters");
		return normalized;
	}
	...
	private String normalizeDescription(String description) {
		...
		if (normalized.length() > 1000)
			throw new IllegalArgumentException("Deck description must not exceed 1000 characters");
		return normalized.isEmpty() ? null : normalized;
	}

	private DeckResponse toResponse(Deck deck) {
		return new DeckResponse(deck.getId(), deck.getName(), deck.getDescription(),
				deck.isPublic(), deck.getStatus(), deck.getCreatedAt(), deck.getUpdatedAt());
	}

	private DeckSummaryResponse toSummary(Deck deck) {
		return new DeckSummaryResponse(deck.getId(), deck.getName(), deck.getDescription(),
				deck.isPublic(), deck.getStatus(), deck.getCreatedAt(), deck.getUpdatedAt());
	}
}
